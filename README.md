...
- **Transaction Management**: View and filter transactions by year and month — the current month loads automatically on page load, with manual filtering still available via Apply Filter. The transactions list is responsive — stacked cards on portrait phones (below the `sm`/640px breakpoint) and a full table on landscape phones and desktop, so no horizontal scrolling is needed on mobile
